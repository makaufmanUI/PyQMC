...
Read data from a QMC5883L magnetic sensor, covering a full turn around the `Z` axis (i.e. on the `X-Y` plane).

During the acquiring phase, it shows a curses interface to give a feedback on how many points were acquired and at what turning angle. 

When enough data is acquired (or when the `Q` key is pressed), it saves a text file with the raw `X`, `Y`, `Z` coordinates.
"""
...
# ------------------------------------------------------------------------
# ------------------------------------------------------------------------
def print_at(x, y, string, attr=curses.A_NORMAL):           # type: ignore
    global stdscr
    try:
...
stdscr = curses.initscr()                                   # type: ignore
...
while True:
    ...
    if x is not None and y is not None:
        ...
        print_at(NEEDLE_X, NEEDLE_Y, "O", curses.A_REVERSE) # type: ignore
        ...
        if sampled < SAMPLES_PER_SECTOR:
            ...
            if completed < 10:
                ...
                attr = curses.A_NORMAL                      # type: ignore
            else:
                completed = '#'
                attr = curses.A_REVERSE                     # type: ignore
